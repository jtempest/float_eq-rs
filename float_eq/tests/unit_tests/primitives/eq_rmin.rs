...
macro_rules! impl_tests {
    ($float:ident) => {
        mod $float {
            ...
            fn check_eq_rmin(a: $float, b: $float, max_diff: $float) {
                assert_float_eq!(a, b, rmin <= max_diff);
                assert_float_eq!(a, b, rmin_all <= max_diff);

                assert!(float_eq!(a, b, rmin <= max_diff));
                assert!(float_eq!(a, b, rmin_all <= max_diff));
                assert!(!float_ne!(a, b, rmin <= max_diff));
                assert!(!float_ne!(a, b, rmin_all <= max_diff));

                assert!(a.eq_rmin(&b, &max_diff));
                assert!(a.eq_rmin_all(&b, &max_diff));
                assert!(!a.ne_rmin(&b, &max_diff));
                assert!(!a.ne_rmin_all(&b, &max_diff));
            }

            fn check_ne_rmin(a: $float, b: $float, max_diff: $float) {
                assert_float_ne!(a, b, rmin <= max_diff);
                assert_float_ne!(a, b, rmin_all <= max_diff);
            ...
            fn check_eq(a: $float, b: $float, max_diff: $float) {
                check_eq_rmin(a, b, max_diff);
                check_eq_rmin(b, a, max_diff);
                check_eq_rmin(-a, -b, max_diff);
                check_eq_rmin(-b, -a, max_diff);
            ...
            fn check_ne(a: $float, b: $float, max_diff: $float) {
                check_ne_rmin(a, b, max_diff);
                check_ne_rmin(b, a, max_diff);
                check_ne_rmin(-a, -b, max_diff);
                check_ne_rmin(-b, -a, max_diff);
